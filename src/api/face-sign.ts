...
export const getFaceSignStatus = async (userId: string): Promise<{ faceSignHash: boolean, faceSignDone: boolean }> => {
...
export const getSessionToken = async (userId: string, key: string, deviceIdentifier: string): Promise<string> => {
  const response = await axiosInstance.post(
