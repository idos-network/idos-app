...
export const getTokenInfo = async (name: string, chainName = 'ethereum') => {
  ...
  const token: Coin | undefined = coinList.find(
    (coin: Coin) => coin.name === name,
  );
  ...
  const tokenContractInfo = await getTokenContractInfo(
    token.platforms[chainName],
    chainName,
  );
  const chainDecimals =
    tokenContractInfo.detail_platforms[chainName].decimal_place;
  return {
...
export const getTokenContractInfo = async (
  address: string,
  chainName = 'ethereum',
) => {
  const coinList = await fetch(
    `https://api.coingecko.com/api/v3/coins/${chainName}/contract/${address}`,
    {
      headers: {
        'x-cg-demo-api-key': apiKey,
      },
...
export const getEthPriceInUSD = async (): Promise<number> => {
  ...
  if (
    window.ethPrice &&
    Date.now() - window.ethPrice.timestamp < CACHE_DURATION
  ) {
