import {
  deserialize as borshDeserialize,
...
export const verifyNearSignature = async (
  ...
  try {
    ...
    const publicKeyBytes = bs58.decode(pieces[1]);
    ...
    const payloadLength = new DataView(signatureBytes.buffer).getUint16(
      0,
      false,
    );
    ...
    const deserializedPayload = borshDeserialize(
      payloadSchema,
      payloadBytes,
    ) as NEP413Payload;
    ...
    const fullPayloadBytes = borshSerialize(
      payloadSchema,
